...
public class Q13Rep {
    ...
    public Q13Rep(@Qualifier(Constant.BEAN_JDBC_TEMPLATE) JdbcTemplate jdbcTemplate) {
    ...
    public List<Q13Do> getList(Q13Query query) {
        ...
        logger.debug(MessageFormat.format("Q13 SQL {0} {1} {2}",SQL_QUERY, minSetlTime,maxSetlTime));
        Map<String, Q13Do> map = new HashMap<>();
        jdbcTemplate.query(SQL_QUERY, new RowCallbackHandler() {
            ...
            public void processRow(ResultSet rs) throws SQLException {
                c = c + 1;
                if(c % 2000 == 0) {
                    logger.debug(MessageFormat.format("Q13 RowCallbackHandler {0}", c));
                }
                ...
                nd.setDedcHospLv(SQLTool.getString(rs, "PSN_TYPE"));
        ...
        List<Q13Do> list = new ArrayList<>(map.values());
        Collections.sort(list);
        return list;
    }

    private Q13Do getCurr(String mdtrtId, String setlId) {
        if(mdtrtId == null || setlId == null || "".equals(mdtrtId) || "".equals(setlId)) {
            return null;
        }
        return jdbcTemplate.queryForObject(SQL_QUERY_SIGNAL, new Q13SignalRowMapper(), mdtrtId, setlId);
    }
