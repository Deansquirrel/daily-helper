...
public class Q18Rep {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Q18Rep.class);

    private static final String SQL_QUERY = "" +
            "SELECT INSU_ADMDVS, MDTRT_ID, SETL_ID " +
            "FROM setl_d " +
    ...
    public Q18Rep(@Qualifier(Constant.BEAN_JDBC_TEMPLATE) JdbcTemplate jdbcTemplate) {
    ...
    public List<Q18Do> getList(Q18Query query) {
        ...
        logger.debug(MessageFormat.format("Q18 SQL {0} {1} {2}",SQL_QUERY, minSetlTime,maxSetlTime));
        Map<String, Q18Do> map = new HashMap<>();
        jdbcTemplate.query(SQL_QUERY, new RowCallbackHandler() {
            ...
            public void processRow(ResultSet rs) throws SQLException {
                c = c + 1;
                if(c % 2000 == 0) {
                    logger.debug(MessageFormat.format("Q18 RowCallbackHandler {0}", c));
                }
                ...
                String k = MessageFormat.format("{0}",nd.getInsuAdmdvs());
        ...
        List<Q18Do> list = new ArrayList<>(map.values());
        Collections.sort(list);
        return list;
    }

    private Q18Do getCurr(String mdtrtId, String setlId) {
        if(mdtrtId == null || setlId == null || "".equals(mdtrtId) || "".equals(setlId)) {
            return null;
        }
        return jdbcTemplate.queryForObject(SQL_QUERY_SIGNAL, new Q18SignalRowMapper(), mdtrtId, setlId);
    }
